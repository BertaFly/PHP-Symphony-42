...
class DefaultController extends Controller
{
    ...
    {
        return $this->render('@ex04/Default/index.html.twig');
    }
    ...
    public function formAction(Request $request)
    {
	    ...
	    if ($form->isSubmitted() && $form->isValid()) {
	        ...
	        $ret = $this->_getOne("SELECT username, email FROM ex04 WHERE `username` = '$username' OR `email` = '$email'");

	        if (!$ret) {
	        	$this->_sendData('ex04', $data);
    			return $this->redirectToRoute('ex04_default_formsuccess');
	        } else {
        ...
        return $this->render('@ex04/Default/form.html.twig', array(
    ...
    {
        $users = $this->_getAll("SELECT * FROM ex04");

        return $this->render('@ex04/Default/success.html.twig', array(
        	'users' => $users,
        ));
    }
    ...
    public function createTableAction()
    {
		...
		$this->pdo = $this->_getConnection();
		$this->_createTable('ex04');

    	return $this->redirectToRoute('ex04_default_form');
    }
    ...
    public function deleteAction($id)
    {
		...
		$this->pdo = $this->_getConnection();
        $ret = $this->_getOne("SELECT id FROM ex04 WHERE `id` = '$id'");

        if ($ret) {
        	$this->_getOne("DELETE FROM ex04 WHERE `id` = '$id'");
	        $this->addFlash(
	            'success',
